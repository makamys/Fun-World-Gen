...
import fwg.noise.OldNoiseGeneratorOctaves2;
import fwg.support.Support;
import fwg.util.CellNoise;
...
public class ManagerFWG extends WorldChunkManager
{
	...
	protected ManagerFWG()
	{
        this.biomeCache = new BiomeCache(this);
        this.biomesToSpawnIn = new ArrayList();
	}

    public ManagerFWG(World par1World, boolean remote, int biomes)
    {
        this();
        ...
        if(remote)
        {
			...
			else if(biomes == 1)
			{
				...
				biomeList_small.addAll(Support.biomes_small);
			}
    ...
    {		
		...
		for(int j1 = 0; j1 < par3; j1++)
		{
			for(int k1 = 0; k1 < par4; k1++)
			{
				...
				temperature[i1] = d3;
				humidity[i1] = d4;
				abiomegenbase[i1++] = BiomeGenBase.plains.biomeID;//getBiomeFromLookup(d3, d4);
			}
		}
